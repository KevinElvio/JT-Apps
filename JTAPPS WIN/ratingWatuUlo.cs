...
{
    public partial class ratingWatuUlo : Form
    {
        private int selectedRating;
        ReviewContext reviewContext;
        public ratingWatuUlo()
        {
        ...
        private void AverageRating()
        {
            using (NpgsqlConnection conn = new NpgsqlConnection("host=localhost;port=5432;database=JT-Apps;user id=postgres;password=timotius"))
            {
                ...
                cmd.Connection = conn;
                cmd.CommandText = "select AVG(rating_tempat) from \"penilaian\" where wisata_id_wisata like'A02'";
